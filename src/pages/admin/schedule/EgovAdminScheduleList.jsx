...
function EgovAdminScheduleList(props) {
  ...
  const drawCalendar = () => {
    ...
    let lastOfThisMonth = getLastDateOfMonth(
      searchCondition.year,
      searchCondition.month
    );
            ...
            if (day !== 0) {
              ...
              if (scheduleList.length > 0) {
                ...
                return (
                  <td key={keyIdx++}>
                    <Link
                      to={{ pathname: URL.ADMIN_SCHEDULE_CREATE }}
                      state={{ iUseDate: mutsUseYearMonth + sDate + "000000" }}
                      className="day"
                      key={keyIdx++}
                    >
                      {day}
                    ...
                    {scheduleList.map((schedule, scheduleIdx) => {
                      let iBeginDate = Number(
                        schedule.schdulBgnde.substring(0, 8)
                      );
                      let iEndDate = Number(
                        schedule.schdulEndde.substring(0, 8)
                      );
                      innerConsole(
                        "scheduleList ",
                        day,
