...
function ManagerStatisticsUser(props) {
    ...
    const getStatistics = (v, i) => {
        ...
        let dimensions = "";
        if(v == "byRegionUser"){
            dimensions = JSON.stringify(["city"]);
        }else{
            if(searchCategory == "day"){
        ...
        EgovNet.requestFetch("/statisticsApi/getStatistics.do", requestOptions, function (resp) {
            if(v == "periodUser"){
                ...
                // userSts.periodUserListMake(rs.sts)
            }else{
                // resp.result.rs.sort(function(a, b) {
                //     var indexA = userSts.global.korCityMap.indexOf(a.korCityName); // a의 지역 인덱스
                //     var indexB = userSts.global.korCityMap.indexOf(b.korCityName); // b의 지역 인덱스
                //
                //     // 지역이 regionOrder 배열에 없으면 정렬에서 무시
                //     if (indexA === -1 && indexB === -1) return 0;
                //     if (indexA === -1) return 1; // a가 없으면 b를 앞에 둠
                //     if (indexB === -1) return -1; // b가 없으면 a를 앞에 둠
                //
                //     return indexA - indexB; // 인덱스에 따라 정렬
                // });
                //
                // userSts.byRegionUserListMake(rs.sts);
            }
        });
    }
    ...
    useEffect(() => {
        if (searchDto.page === "userSts") {
            getStatistics("periodUser", "");
        }
    }, [searchDto]);
