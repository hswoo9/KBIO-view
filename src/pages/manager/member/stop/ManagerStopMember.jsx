...
function StopMemberList(props) {
    ...
    const activeEnter = (e) => {
        if (e.key === "Enter") {
            e.preventDefault();
            getstopMemberList(searchDto);
        }
    };

    const setStopMemberApproval = (userSn) => {
        const setStopMemberApporvalUrl = "/memberApi/setStopMemberApproval";

        Swal.fire({
            title: `<span style="font-size: 14px; line-height: 0.8;">
                    이용정지 된 회원을 정지해제 할 경우 회원은<br>
                    홈페이지 접속 및 서비스 이용이 가능합니다.<br>
                    해당 계정의 정지상태를 해제 하시겠습니까?
                </span>
            `,
            ...
            if(result.isConfirmed) {
                ...
                EgovNet.requestFetch(setStopMemberApporvalUrl, requestOptions, (resp) => {
    ...
    const getstopMemberList = useCallback(
        (searchDto) => {
            const stopMemberListURL = "/memberApi/getStopMemberList.do";
            const requestOptions = {
                method: "POST",
                headers: {
                    "Content-type": "application/json",
            ...
            EgovNet.requestFetch(
                ...
                (resp) => {
                    ...
                    resp.result.getStopMemberList.forEach(function (item, index) {
                        if (index === 0) dataList = [];

                        const totalItems = resp.result.getStopMemberList.length;
                        ...
                        dataList.push(
                            <tr key={item.userSn}>
                                ...
                                <td>
                                    <button type="button"
                                            onClick={() => {
                                                setStopMemberApproval(item.userSn);
                                            }}
                                    >
                                        해제
                                    </button>
    ...
    return (
        <div id="container" className="container layout cms">
            <ManagerLeft/>
            <div className="inner">
                <h2 className="pageTitle"><p>이용정지</p></h2>
